...
class LRUCache(BaseCaching):
    ...
    def put(self, key, item):
        """Stores an item in the cache. If the cache is full, removes the least recently used item.
        """
        if key is None or item is None:
            return
        if key not in self.cache_data:
            if len(self.cache_data) + 1 > BaseCaching.MAX_ITEMS:
                lru_key, _ = self.cache_data.popitem(True)  # Discard the least recently used item
                print("DISCARD:", lru_key)
            self.cache_data[key] = item
            self.cache_data.move_to_end(key, last=False)  # Place the new item at the start
        else:
            self.cache_data[key] = item

    def get(self, key):
        """Returns the item associated with the specified key and marks it as recently used.
        """
